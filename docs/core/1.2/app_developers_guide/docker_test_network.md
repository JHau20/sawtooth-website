...
3. (Optional) You can run the following Sawtooth commands to show the other nodes on the network.
	...
	3. Run `sawtooth peer list` to show the peer of a particular node. For example, the following command specifies the REST API on the first node, so it displays t	he first node's peers.
	...
	3. Run `sawnet peers list` to display a complete graph of peers on the network (available in Sawtooth release 1.1 and later).
